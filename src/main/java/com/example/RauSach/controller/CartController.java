...
public class CartController {
    ...
    public String home(Model model,@PathVariable Long id){
        ...
        List<CartPet> cartItems = cartPetService.getCartItems();
        List<CartProduct> cartProducts = cartPetService.getProductItems();
...
// Calculate the total price
        ...
        long totalPricePet = cartItems.stream()
                .filter(cartItem -> cartItem.getPet() != null)
                .mapToLong(cartItem -> cartItem.getPet().getPrice().longValue() * cartItem.getQuantity())
                .sum();
